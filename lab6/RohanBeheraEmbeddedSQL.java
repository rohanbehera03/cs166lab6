...
public class EmbeddedSQL {
   ...
   public static void Query4(EmbeddedSQL esql){
      try{
         String query = "SELECT DISTINCT S.sname, MAX(C.cost) FROM Suppliers S, Parts P, Catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND S.sid IN (SELECT S.sid FROM Suppliers S, Parts P, Catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND P.color = 'Red' INTERSECT SELECT S.sid FROM Suppliers S, Parts P, Catalog C WHERE S.sid = C.sid AND P.pid = C.pid AND P.color = 'Green') GROUP BY S.sid;";         
         
         int rowCount = esql.executeQuery(query);
         System.out.print ("total row(s): " + rowCount);
      }catch(Exception e) {
         System.err.println (e.getMessage());
      }
